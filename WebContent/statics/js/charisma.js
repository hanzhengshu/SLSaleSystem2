$(document).ready(function(){
	...
	$("#modifySavePassword").click(function(){
		...
		if("" == oldpwd){
			...
			$("#oldpassword").focus();     //.focus() 鼠标指到出错的地方
		}else if("" == newpwd){
			tip.css("color","red");
	...
	$('.modifypwd').click(function(e){
		...
		$('#aginpassword').val('');
		$('#modifypwdtip').html('');
		e.preventDefault();		//preventDefault() 方法阻止元素发生默认的行为（屏蔽标签的所有默认事件）
		//模态框（Modal）是覆盖在父窗体上的子窗体。
		$('#myModal').modal('show');  //显示出隐藏的元素 -> show，指的是点击的时候触发打开窗口。
	});
	/**modifypassword end*/
	...
	/**
	 * menuList add start
	 * 	添加menuList   eval() 函数可计算某个字符串，并执行其中的的 JavaScript 代码。
	...
	for(var i = 0; i < json.length; i++){
		...
		//config main menu	主菜单						onclick:当点击按钮时执行一段 JavaScript：	<style> 标签用于为 HTML 文档定义样式信息。 cursor:pointer鼠标放到位置显示手型的光标
	...
	$("#menus").append(result);//在指定元素的结尾插入内容。在head.jsp中id为menus结尾插入内容
	/**menulist end*/
